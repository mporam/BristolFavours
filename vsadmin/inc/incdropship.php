...
if(@$_POST["act"]=="domodify"){
	$sSQL = "UPDATE dropshipper SET dsEmail='" . escape_string(unstripslashes(@$_POST["email"])) . "'," .
		"dsName='" . escape_string(unstripslashes(@$_POST["name"])) . "'," .
		"dsAddress='" . escape_string(unstripslashes(@$_POST["address"])) . "'," .
		"dsCity='" . escape_string(unstripslashes(@$_POST["city"])) . "'," .
		"dsState='" . escape_string(unstripslashes(@$_POST["state"])) . "'," .
		"dsCountry='" . escape_string(unstripslashes(@$_POST["country"])) . "'," .
		"dsZip='" . escape_string(unstripslashes(@$_POST["zip"])) . "'," .
		"dsAction=" . escape_string(unstripslashes(@$_POST["dsAction"])) . "," .
		"dsEmailHeader='" . escape_string(unstripslashes(str_replace(array('<br>', '<br/>', '<br />'), '%nl%', @$_POST['dsEmailHeader']))) . "' " .
		"WHERE dsID=" . escape_string(unstripslashes(@$_POST["dsID"]));
	...
	$sSQL = "INSERT INTO dropshipper (dsEmail,dsName,dsAddress,dsCity,dsState,dsCountry,dsZip,dsAction,dsEmailHeader) VALUES (" .
		...
		"" . escape_string(unstripslashes(@$_POST["dsAction"])) . "," .
		"'" . escape_string(str_replace(array('<br>', '<br/>', '<br />'), '%nl%', unstripslashes(@$_POST['dsEmailHeader']))) . "')";
	mysql_query($sSQL) or print(mysql_error());
	$dorefresh=TRUE;
}elseif(@$_POST["act"]=="delete"){
	$sSQL = "DELETE FROM dropshipper WHERE dsID=" . trim(@$_POST["id"]);
	mysql_query($sSQL) or print(mysql_error());
	$sSQL = "UPDATE products SET pDropship=0 WHERE pDropship=" . trim(@$_POST["id"]);
	mysql_query($sSQL) or print(mysql_error());
	$dorefresh=TRUE;
}
if($dorefresh){
