...
function getrecpt($oid){
	global $xxOrdId,$xxBilAdd,$xxShpAdd,$xxPhone,$xxEmail,$xxShpMet,$xxWtIns,$xxCerCLo,$xxSatDeR,$xxAddInf,$xxCODets,$xxCOName,$xxCOUPri,$xxQuant,$xxTotal,$xxSubTot,$xxDscnts,$xxShipHa,$xxFree,$xxShippg,$xxHndlg,$xxStaTax,$xxHST,$xxCntTax,$xxGndTot,$ordGrandTotal;
	global $extraorderfield1,$extraorderfield2,$extracheckoutfield1,$extracheckoutfield2,$combineshippinghandling,$hideoptpricediffs,$digidownloads,$willpickuptext,$emailcr,$loyaltypoints;
	$recpt = '';
	...
	if($rs=mysql_fetch_assoc($result)){
		...
		if($ordShipType!=''){
			...
			if(($rs['ordComLoc'] & 2)==2 && @$willpickuptext!=$ordShipType) $shiptext .= $xxWtIns;
			$shiptext .= '<br />';
		...
		if($ordAddInfo!=''){
			...
			$recpt .=  '	  <td align="left" colspan="3">'.$ordAddInfo.'</td>';
			$recpt .=  '	</tr>'.$emailcr;
		}
		...
		$recpt .=  '	<td class="receiptheading" width="14%" height="25" align="right"><strong>'.$xxCOUPri.'</strong></td>';
		$recpt .=  '	<td class="receiptheading" width="14%" height="25" align="right"><strong>'.$xxQuant.'</strong></td>';
		$recpt .=  '	<td class="receiptheading" width="14%" height="25" align="right"><strong>'.$xxTotal.'</strong></td>';
		$recpt .=  '  </tr>'.$emailcr;
	}
	mysql_free_result($result);
	$sSQL = "SELECT cartID,cartProdID,cartProdName,cartProdPrice,cartQuantity FROM cart WHERE cartOrderID='" . escape_string($oid) . "' ORDER BY cartID";
	$result = mysql_query($sSQL) or print(mysql_error());
	while($alldata=mysql_fetch_assoc($result)){
		...
		while($rs2=mysql_fetch_assoc($opts)){
			...
			$theoptions .= '<td class="receiptoption" align="left"><span style="font-size:0.82em">&nbsp;- ' . str_replace(array("\r\n",$emailcr),array("<br />","<br />"),strip_tags($rs2['coCartOption'])) . '</span></td>';
			$theoptions .= '<td class="receiptoption" align="right"><span style="font-size:0.82em">' . ($rs2['coPriceDiff']==0 || @$hideoptpricediffs==TRUE ? '- ' : FormatEuroCurrency($rs2['coPriceDiff'])) . '</span></td>';
			$theoptions .= '<td class="receiptoption">&nbsp;</td>';
			$theoptions .= '<td class="receiptoption" align="right"><span style="font-size:0.82em">' . ($rs2['coPriceDiff']==0 || @$hideoptpricediffs==TRUE ? '- ' : FormatEuroCurrency($rs2['coPriceDiff']*$alldata['cartQuantity'])) . '</span></td>';
		...
		$recpt .=  '	<td class="cobhl receipthl" align="left">'.$alldata['cartProdName'].'</td>';
		$recpt .=  '	<td class="cobhl receipthl" align="right">'.(@$hideoptpricediffs==TRUE ? FormatEuroCurrency($alldata['cartProdPrice'] + $theoptionspricediff) : FormatEuroCurrency($alldata['cartProdPrice'])).'</td>';
		$recpt .=  '	<td class="cobhl receipthl" align="right">'.$alldata['cartQuantity'].'</td>';
		$recpt .=  '	<td class="cobhl receipthl" align="right">'.(@$hideoptpricediffs==TRUE ? FormatEuroCurrency(($alldata['cartProdPrice'] + $theoptionspricediff)*$alldata['cartQuantity']) : FormatEuroCurrency($alldata['cartProdPrice']*$alldata['cartQuantity'])).'</td>';
		$recpt .=  '  </tr>'.$emailcr;
		$recpt .=  $theoptions;
	}
	$recpt .=  '	  <tr>';
	...
	if($rs['ordDiscount']>0){
		...
		$recpt .=  '	<td align="right"><span style="color:#FF0000">'.FormatEuroCurrency($rs['ordDiscount']).'</span></td>';
		$recpt .=  ' </tr>'.$emailcr;
	}
...
function do_order_success($sorderid,$sEmail,$sendstoreemail,$doshowhtml,$sendcustemail,$sendaffilemail,$sendmanufemail){
	global $thereference,$emlNl,$htmlemails,$extraorderfield1,$extraorderfield2,$extraorderfield3,$shipType,$emailheader,$emailfooter,$emailencoding,$hideoptpricediffs,$xxWtIns,$ordGrandTotal,$ordID,$digidownloads,$dropshipfooter,$dropshipheader,$digidownloademail,$xxPrint,$dropshipsubject,$xxHST,$encodecustomeremailsubject,$imgcontinueshopping,$imgprintversion,$giftcertificateid,$dateformatstr,$recpt,$receiptheader;
	...
	global $ordStateTax,$ordHSTTax,$ordCountryTax,$ordShipping,$ordHandling,$ordCity,$ordState,$ordCountry,$ordEmail,$affilID,$xxDigPro,$thankspagecontinue,$willpickuptext,$xxManRev,$emailcr,$loyaltypoints,$xxLoyPoi;
	if(@$htmlemails==TRUE){
		$emlNl = '<br />'.$emailcr;
		$xxThkYou='';
	}else
	...
	$ordClientID=0;
	$ndropshippers=0;
	$sSQL = "SELECT ordID,ordName,ordLastName,ordAddress,ordAddress2,ordCity,ordState,ordZip,ordCountry,ordEmail,ordPhone,ordShipName,ordShipLastName,ordShipAddress,ordShipAddress2,ordShipCity,ordShipState,ordShipZip,ordShipCountry,ordShipPhone,ordPayProvider,ordAuthNumber,ordTotal,ordDate,ordStateTax,ordCountryTax,ordHSTTax,ordHandling,ordShipping,ordAffiliate,ordDiscount,ordDiscountText,ordComLoc,ordExtra1,ordExtra2,ordShipExtra1,ordShipExtra2,ordCheckoutExtra1,ordCheckoutExtra2,ordSessionID,ordAddInfo,ordShipType,payProvID,ordClientID,loyaltyPoints FROM orders LEFT JOIN payprovider ON payprovider.payProvID=orders.ordPayProvider WHERE ordAuthNumber<>'' AND ordID='" . escape_string($sorderid) . "'";
	$result = mysql_query($sSQL) or print(mysql_error());
	if(mysql_num_rows($result) > 0){
		...
		if($rs['ordShipType']=='MODWARNOPEN'){
			...
			print imageorbutton(@$imgcontinueshopping,'&nbsp;'.$xxCntShp.'&nbsp;','',(@$thankspagecontinue!=''?$thankspagecontinue:$storeurl), FALSE).'&nbsp;';
			print '</div>';
			$success=FALSE;
			mysql_free_result($result);
			return;
		...
		if($ordShipType != ''){
			...
			if(($rs['ordComLoc'] & 2)==2 && @$willpickuptext!=$ordShipType) $orderText .= $xxWtIns;
			$orderText .= '<br />';
		...
		$orderText = 'Cannot find customer details for order id: ' . $sorderid . '<br />';
	...
	$reviewlinks='';
	$loyaltypointtotal=0;
	$sSQL = 'SELECT cartProdId,cartProdName,cartProdPrice,cartQuantity,cartID,pDropship,pDisplay,pStaticPage'.(@$digidownloads==TRUE?',pDownload':'')." FROM cart LEFT JOIN products ON cart.cartProdId=products.pID WHERE cartOrderID='" . escape_string($sorderid) . "' ORDER BY cartID";
	$result = mysql_query($sSQL) or print(mysql_error());
	if(mysql_num_rows($result) > 0){
		while($rs = mysql_fetch_assoc($result)){
			if($rs['cartProdId']==$giftcertificateid){
				...
				if($sendcustemail){
					...
					if($rs2 = mysql_fetch_assoc($result2)){
						...
						$emailBody = replaceemailtxt($emailBody, '%message%', trim($rs2['gcMessage']));
			...
			if(is_null($rs['pStaticPage']))
				$thelink = '';
			elseif($rs['pDisplay']==0)
				$thelink = '';
			elseif($rs['pStaticPage']!=0)
				$thelink = $storeurl . cleanforurl($rs['cartProdName']).'.php?review=true';
			else
				$thelink = $storeurl . 'proddetail.php?prod='.$rs['cartProdId'].'&review=true';
			...
			$orderText .= $xxUnitPr . ': ' . (@$hideoptpricediffs==TRUE ? FormatEmailEuroCurrency($rs['cartProdPrice'] + $theoptionspricediff) : FormatEmailEuroCurrency($rs['cartProdPrice'])) . '<br />';
		...
		$orderText .= "Cannot find order details for order id: " . $sorderid . '<br />';
	}
	mysql_free_result($result);
	if(@$loyaltypoints!='' && $orderloyaltypoints==0){
	...
	$emailheader = replaceemailtxt($emailheader, '%reviewlinks%', $reviewlinks);
	$receiptheader = replaceemailtxt($receiptheader, '%reviewlinks%', $reviewlinks);
