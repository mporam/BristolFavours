...
<?php
	...
	if($success){
		...
		print '<script language="javascript" type="text/javascript">document.getElementById(\'step2span\').innerHTML=\'<strong>Failed!</strong>\';document.getElementById(\'donebutton\').style.display=\'block\';</script>';
	}
}elseif(@$_POST["upsstep"]=="3"){
	splitname(@$_POST['contact'], $firstname, $lastname);
	$sXML='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:v2="http://fedex.com/ws/registration/v2"><soapenv:Header/><soapenv:Body>' .
		...
		"<v2:ClientDetail>" .
		"<v2:AccountNumber>" . trim(@$_POST['fedexaccount']) . "</v2:AccountNumber>" .
		...
		"<v2:BillingAddress>" .
		"<v2:StreetLines>" . trim(@$_POST['address']) . "</v2:StreetLines>" .
		"<v2:City>" . trim(@$_POST['city']) . "</v2:City>" .
		"<v2:StateOrProvinceCode>" . trim(@$_POST['usstate']) . "</v2:StateOrProvinceCode>" .
		"<v2:PostalCode>" . trim(@$_POST['postcode']) . "</v2:PostalCode>" .
		"<v2:CountryCode>" . trim(@$_POST['country']) . "</v2:CountryCode>" .
		"</v2:BillingAddress>" .
		...
		"</v2:PersonName>" .
		"<v2:CompanyName>" . trim(@$_POST['company']) . "</v2:CompanyName>" .
		"<v2:PhoneNumber>" . trim(@$_POST['telephone']) . "</v2:PhoneNumber>" .
		"<v2:EMailAddress>" . trim(@$_POST['email']) . "</v2:EMailAddress>" .
		"</v2:Contact>" .
		"<v2:Address>" .
		"<v2:StreetLines>" . trim(@$_POST['address']) . "</v2:StreetLines>" .
		"<v2:City>" . trim(@$_POST['city']) . "</v2:City>" .
	...
	if($success){
		$sXML='<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">' .
			...
			"</WebAuthenticationDetail>" .
			"<ClientDetail>" .
			"<AccountNumber>" . trim(@$_POST['fedexaccount']) . "</AccountNumber>" .
			"<MeterNumber/>" .
			...
			"<AccountNumber>" . trim(@$_POST['fedexaccount']) . "</AccountNumber>" .
			"<Contact>" .
			"<PersonName>" . trim(@$_POST['contact']) . "</PersonName>" .
			"<CompanyName/>" .
			"<PhoneNumber>" . trim(@$_POST['telephone']) . "</PhoneNumber>" .
			"<FaxNumber/>" .
			"<EMailAddress>" . trim(@$_POST['email']) . "</EMailAddress>" .
			"</Contact><Address>" .
			"<StreetLines>" . trim(@$_POST['address']) . "</StreetLines>" .
			"<City>" . trim(@$_POST['city']) . "</City>" .
			"<StateOrProvinceCode>" .trim( @$_POST['usstate']) . "</StateOrProvinceCode>" .
			"<PostalCode>" . trim(@$_POST['postcode']) . "</PostalCode>" .
			"<CountryCode>" . trim(@$_POST['country']) . "</CountryCode>" .
			"</Address></Subscriber>" .
...
<?php
	if($success){
		$sSQL = "UPDATE admin SET FedexAccountNo='" . trim($_POST["fedexaccount"]) . "',FedexMeter='" . $fedexmeter . "',FedexUserKey='" . $userkey . "',FedexUserPwd='" . $userpwd . "'";
