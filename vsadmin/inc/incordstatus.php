...
if(@$_POST["act"]=="domodify"){
	for($index=0; $index < 30; $index++){
		...
		if($statusid != ""){
			$statPrivate = escape_string(unstripslashes(@$_POST["privstatus" . $index]));
			$statPublic = escape_string(unstripslashes(@$_POST["pubstatus" . $index]));
			...
			for($index2=2; $index2 <= $adminlanguages+1; $index2++){
				if(($adminlangsettings & 64)==64) $sSQL .= ",statPublic" . $index2 . "='" . escape_string(unstripslashes(@$_POST["pubstatus" . $index . "x" . $index2])) . "'";
			}
