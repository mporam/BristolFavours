...
if(@$_POST["act"]=="dumpaffiliate"){
	...
	while($rs = mysql_fetch_assoc($result)){
		if((int)$rs['pStockByOpts'] != 0){
			$result2 = mysql_query("SELECT optID,optGrpName,optName,optStock FROM optiongroup INNER JOIN options ON optiongroup.optGrpID=options.optGroup INNER JOIN prodoptions ON options.optGroup=prodoptions.poOptionGroup WHERE prodoptions.poProdID='" . escape_string($rs["pID"]) . "'") or print(mysql_error());
	...
	$fieldlist .= ',pSection,pPrice,pWholesalePrice,pListPrice,pShipping,pShipping2,pWeight,pDisplay,pSell,pExemptions,pInStock,pDims,pTax,pDropship';
	if(@$digidownloads==TRUE) $fieldlist .= ',pDownload';
	$fieldlist .= ',pStaticPage,pStockByOpts,pRecommend,pOrder,pSKU,pManufacturer,pSearchParams,pDescription';
	for($index=2; $index <= $adminlanguages+1; $index++){
	...
	while($rs = mysql_fetch_assoc($result)){
		...
		if(@$dumpccnumber){
			if($sslok==FALSE){
				...
				$result2 = mysql_query("SELECT ordCNum,ordPayProvider FROM orders WHERE ordID=" . $rs["ordID"]) or print(mysql_error());
	...
	$result = mysql_query($sSQL2 . ' ORDER BY ordID' . ($hasdetails?',cartID':'')) or print(mysql_error());
	print '"OrderID",';
	...
	while($rs = mysql_fetch_assoc($result)){
		...
		if($hasdetails){
			$theOptions = "";
			$thePriceDiff = 0;
			$result2 = mysql_query("SELECT coPriceDiff,coOptGroup,coCartOption FROM cartoptions WHERE coCartID=" . $rs['cartID'] . ' ORDER BY coID') or print(mysql_error());
			while($rs2 = mysql_fetch_assoc($result2)){
